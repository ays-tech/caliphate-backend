...
// Combined multer options for the create endpoint (cover + bookFile)
const createBookUploadOptions = {
  ...bookFileUploadOptions, // use the larger 100MB limit
};
...
export class BooksController {
  ...
  findAll(
    ...
    return this.booksService.findAll({
      search, scholarId, type,
      page:  page  ? Math.max(1, parseInt(page))  : 1,
      limit: limit ? Math.min(50, parseInt(limit)) : 12,
    });
  }
  ...
  findMostRead(@Query('limit') limit?: string) {
    return this.booksService.findMostRead(
  ...
  findRecent(@Query('limit') limit?: string) {
    return this.booksService.findRecent(
      limit ? Math.min(20, parseInt(limit)) : 6,
    );
  }
  ...
  // Accepts two optional file fields in one multipart form:
  //   cover    — book cover image (max 5 MB, images only)
  //   bookFile — actual book file: PDF / EPUB / image (max 100 MB)
  @Post()
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.ADMIN, Role.SUPER_ADMIN)
  @UseInterceptors(
