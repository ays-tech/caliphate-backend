...
// ── Books ─────────────────────────────────────────────────────────────
// Each entry maps to a scholar by index in the SCHOLARS array above
const BOOKS = [
  // Ibn Khaldun (index 0)
  {
    scholarIndex: 0,
  ...
  },
  {
    ...
    description:
      'A collection of 1,322 hadiths compiled by Imam al-Bukhari focusing exclusively on Islamic ethics and social conduct — covering topics such as respect for parents, kindness to neighbours, treatment of servants, hospitality, and noble character. Less well-known than the Sahih but deeply practical, it provides a rich portrait of the Prophet\'s ﷺ moral teachings.',
    type: BookType.PUBLISHED,
...
const EVENTS = [
  {
    ...
    description: 'A weekly gathering for the study and recitation of the Holy Qur\'an, open to all levels. This week\'s focus: Surah Al-Baqarah verses 255–257 (Ayat al-Kursi and the following verses).',
    daysFromNow: 3,
  },
  {
    title: 'Islamic History Lecture Series: The Golden Age',
    description: 'A monthly lecture series exploring the intellectual achievements of the Islamic Golden Age (8th–13th centuries). This session covers the House of Wisdom in Baghdad and its role in preserving and advancing human knowledge.',
    daysFromNow: 10,
  },
  {
    title: 'Fiqh Study Circle: Purification & Prayer',
    description: 'An in-depth study of Islamic jurisprudence covering the chapters of Taharah (purification) and Salah (prayer) according to the major schools of law. Suitable for beginners and intermediate students.',
    daysFromNow: 17,
  },
  {
    title: 'New Book Launch: Translations of Ibn Khaldun',
    description: 'Celebrating the addition of newly translated and annotated volumes of Ibn Khaldun\'s Muqaddimah to the Makhtaba collection. Join us for a discussion of his theories of civilisation and their relevance today.',
    daysFromNow: 24,
  },
  {
    title: 'Youth Islamic Knowledge Competition',
    description: 'An annual knowledge competition for young Muslims aged 12–25 covering Qur\'anic recitation, hadith, Islamic history, and jurisprudence. Registration is now open through the library.',
    daysFromNow: 45,
  },
];
...
async function main() {
  ...
  for (let i = 0; i < SCHOLARS.length; i++) {
    ...
    const pictureUrl = s.pictureUrl || `media/general_${(i % 3) + 1}.jpg`;

    if (existing) {
      console.log(`  ✓  Scholar already exists: ${s.name}`);
      scholarRecords.push(existing);
    } else {
      const record = await prisma.scholar.create({
        data: {
          ...s,
          pictureUrl,
        },
  ...
  for (let i = 0; i < BOOKS.length; i++) {
    ...
    const coverUrl = b.coverUrl || `media/general_${(i % 3) + 1}.jpg`;
    ...
    await prisma.book.create({
      data: {
        ...
        coverUrl,
        type:         b.type,
        status:       BookStatus.APPROVED,  // auto-approve seed books
        readCount:    Math.floor(Math.random() * 480) + 20, // realistic read counts
